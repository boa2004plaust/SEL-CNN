...
def read_data(file):
    ...
    n_samples, n_width, n_height = data['data'].shape  # (N, 128, 128)
    d = data['data']  # (n_samples, 128, 128)

    d = np.transpose(d, (0, 1, 2))
    np_data = d.reshape((n_samples, 1, n_width, n_height))  # (n_samples*n_frames, 4, 128)

    # dd = d.reshape((n_samples//400, 4, 100, n_width, n_height))
    # d = np.transpose(dd, (0, 2, 1, 3, 4))
    # np_data = d.reshape((n_samples//4, 4, n_width, n_height))  # (n_samples*n_frames, 4, 128)
    # print(np_data.shape, np_data[0, 0, :5, :5])

    if file.find('NLOS') > 0:
        label = np.zeros((n_samples, 1), dtype=int)
    else:
        label = np.ones((n_samples, 1), dtype=int)
    np_label = label.reshape(n_samples,)
...
def prepare_traindata(scene):
    np_pos_data, np_pos_label = read_data('../data/'+scene+'/MWT_data_LOS_train.mat')
    np_neg_data, np_neg_label = read_data('../data/'+scene+'/MWT_data_NLOS_train.mat')
...
def prepare_testdata(scene):
    np_pos_data, np_pos_label = read_data('../data/'+scene+'/MWT_data_LOS_test.mat')
    np_neg_data, np_neg_label = read_data('../data/'+scene+'/MWT_data_NLOS_test.mat')
    data = np.concatenate((np_pos_data, np_neg_data), axis=0)
    label = np.concatenate((np_pos_label, np_neg_label), axis=0)
    return data, label
...
class DataPreProcess(nn.Module):
    ...
    def forward(self, x):
        # x: numpy data with size (n_samples*n_frames, 4, 128)
        data_trans = x
...
class TrainDataFeeder(Dataset):
    def __init__(self, _data, _label):
        ...
        ndata = _data.shape[0]   # (n_samples, 1, 128, 128)
        dpp = DataPreProcess(ndata)
        _data_smooth = dpp(_data)  # (n_samples, 1, 128, 1288)
...
def parser_args():
    ...
    parser.add_argument('--scene', type=str, default='scene1', help='[scene1, scene2]')
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        DATA_MEAN = train_data.mean(0).mean(0).mean(0).mean(0)
        print(DATA_MEAN)
